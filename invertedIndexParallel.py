...
class InvertedIndex:
    ...
    def serialize(self, output_file="inverted_index.json"):
    ...
    @classmethod
    def deserialize(cls, input_file="inverted_index.json"):
        """Deserialize the inverted index from a JSON file."""
        try:
            with open(input_file, 'r', encoding='utf-8') as file:
                index_data = json.load(file)
            inverted_index = cls()
            inverted_index.index = defaultdict(lambda: defaultdict(int), index_data)
...
if __name__ == "__main__":
    ...
    # Example of deserializing the index
    deserialized_index = InvertedIndex.deserialize()
    print(deserialized_index.search("Frankenstein"))
    print(deserialized_index.search("Contents"))
    print(deserialized_index.search("Title"))
